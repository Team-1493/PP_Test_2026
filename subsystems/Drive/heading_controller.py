...
class HeadingController(Subsystem):
    ...
    def calulateRotationRate(self,stick_rot):
        ...
        elif self.state or  abs(speed)>0.03 :
...
#              "   rot_T: ",f"{self.targetRotation:.4f}", "   state: ", self.state, "   stick_rot F: ",f"{stick_rot:.4f}" ) 
    ...
    def setup(self):
        constants = ConstantValues.HeadingControllerConstants

        self.tp = TrapezoidProfile.Constraints(
            constants.HEADINGCONTROLLER_VMAX, 
            constants.HEADINGCONTROLLER_AMAX) 
        
        self.headingController = ProfiledPIDController(
            constants.HEADINGCONTROLLER_KP, 
            0, 
            constants.HEADINGCONTROLLER_KD,
            self.tp)
        
        self.headingController.enableContinuousInput(-pi, pi)
        self.headingController.setTolerance(
            constants.HEADINGCONTROLLER_TOLERANCE)  #1 degree
        
        # Measured rotation rate at which we assume robot not rotating
        # Need to tune this carefullly - balance between having setpoint drift 
        # and not complating an auto rotation
        self.headingRateTolerance = constants.HEADINGCONTROLLER_RATE_TOLERANCE 
